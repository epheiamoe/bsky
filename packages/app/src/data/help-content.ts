...
- 設定 → AI でAPIキーを設定`,
    ...
    tips: [
      ...
      { icon: 'hash', en: 'Use /view from sidebar chat to inject page context', zh: '从侧边栏对话使用 /view 注入页面上下文', ja: 'サイドバーチャットから /view でページコンテキストを注入' },
...
**注意：** これらのショートカットはTUI固有です。PWAは標準のブラウザインタラクション（クリック、タップ、キーボードナビゲーション）を使用します。`,
...

  {
    ...
    summary: {
      en: 'Inject page context into AI chat with /view',
      zh: '使用 /view 将页面上下文注入 AI 对话',
      ja: '/view でページコンテキストをAIチャットに注入',
    },
    detail: {
      en: `The \`/view\` command manually injects the post or profile you're currently browsing into your AI chat message.

**When you DON'T need /view:**
Clicking the **AI button** on a post opens AI chat with context already injected automatically — the AI already knows what post you're looking at. No command needed.

**When you DO need /view:**
If you open AI chat from the **sidebar** (no specific post context), the AI doesn't know what page you're on. Use \`/view\` to manually inject the current page context.

**How to use:**
1. Browse to a post or profile page
2. Open AI chat from the sidebar
3. Type \`/view\` followed by your question — e.g. \`/view explain this\`
4. The AI receives your current page context (as a \`<currently_viewing>\` tag) plus your message

**Example:**
\`/view explain this\` → The AI sees the post/profile you're viewing and your question "explain this".

**How it works under the hood:**
\`/view\` injects a \`<currently_viewing>\` XML tag into your message containing the current page content. This is a **user-level** injection (unlike the AI button, which uses system-level context).

**Note:** PWA-only. The TUI AI has direct tool access to Bluesky data instead.`,
      zh: `\`/view\` 命令可手动将你当前浏览的帖子或资料注入 AI 对话消息中。

**什么时候不需要 /view：**
点击帖子上的 **AI 按钮** 会自动打开 AI 对话，并自动注入上下文——AI 已经知道你在看哪个帖子，无需任何命令。

**什么时候需要 /view：**
如果你从 **侧边栏** 打开 AI 对话（没有特定帖子上下文），AI 不知道你在哪个页面。此时使用 \`/view\` 手动注入当前页面上下文。

**使用方法：**
1. 浏览到一个帖子或用户主页
2. 从侧边栏打开 AI 对话
3. 输入 \`/view\` 后跟你的问题——例如 \`/view 解释一下这个\`
4. AI 会收到你的当前页面上下文（以 \`<currently_viewing>\` 标签形式）以及你的消息

**示例：**
\`/view 解释一下这个\` → AI 会看到你正在浏览的帖子/资料以及你的问题"解释一下这个"。

**底层原理：**
\`/view\` 会在你的消息中注入一个 \`<currently_viewing>\` XML 标签，包含当前页面内容。这是 **用户级** 注入（不同于 AI 按钮使用的系统级上下文）。

**注意：** 仅限 PWA。TUI 的 AI 可以直接通过工具访问 Bluesky 数据。`,
      ja: `\`/view\` コマンドは、現在閲覧中の投稿やプロフィールをAIチャットメッセージに手動で注入します。

**/view が不要な場合：**
投稿の **AIボタン** をクリックすると、コンテキストが自動注入された状態でAIチャットが開きます——AIは既にどの投稿を見ているか知っています。コマンドは不要です。

**/view が必要な場合：**
**サイドバー** からAIチャットを開いた場合（特定の投稿コンテキストなし）、AIはどのページにいるか知りません。その場合、\`/view\` で現在のページコンテキストを手動注入します。

**使い方：**
1. 投稿またはプロフィールページを閲覧
2. サイドバーからAIチャットを開く
3. \`/view\` の後に質問を入力——例：\`/view これを説明して\`
4. AIが現在のページコンテキスト（\`<currently_viewing>\` タグとして）とメッセージを受け取る

**例：**
\`/view これを説明して\` → AIが閲覧中の投稿/プロフィールと質問「これを説明して」を受け取る。

**内部仕組み：**
\`/view\` はメッセージに現在のページコンテンツを含む \`<currently_viewing>\` XMLタグを注入します。これは **ユーザー レベル** の注入です（AIボタンのシステムレベルコンテキストとは異なります）。

**注意：** PWAのみ。TUIのAIはBlueskyデータに直接ツールアクセスできます。`,
    },
    tips: [
      { icon: 'mouse-pointer-click', en: 'AI button on a post: context is automatic — no /view needed', zh: '帖子上的 AI 按钮：上下文自动注入，无需 /view', ja: '投稿のAIボタン：コンテキストは自動注入——/view不要' },
      { icon: 'hash', en: 'Sidebar AI chat: use /view to inject current page context', zh: '侧边栏 AI 对话：使用 /view 注入当前页面上下文', ja: 'サイドバーのAIチャット：/view で現在のページコンテキストを注入' },
      { icon: 'code', en: '/view injects a <currently_viewing> tag into your message', zh: '/view 在消息中注入 <currently_viewing> 标签', ja: '/view はメッセージに <currently_viewing> タグを注入' },
      { icon: 'info', en: 'PWA-only — TUI AI has direct tool access to Bluesky data', zh: '仅限 PWA——TUI 的 AI 可直接通过工具访问数据', ja: 'PWAのみ——TUIのAIはBlueskyデータに直接ツールアクセス' },
    ],
  },
