...
function useAuth(): {
  ...
  loading: boolean;
  error: string | null;
  errorLog: LoginErrorDetail | null;    // 新增：详细错误原因
...
**新增 `errorLog`**：当登录失败时，如果异常携带 `cause` 且为 `LoginErrorDetail` 类型，`errorLog` 返回该详细结构体，供 UI 显示具体错误码或服务端提示。
...
function useThread(
  ...
  isLiked: (uri: string) => boolean;
  isReposted: (uri: string) => boolean;
  getPostView: (uri: string) => PostView | undefined;   // 新增：PostView 查找
}
```

**`FlatLine` 接口**：`depth`（负数表示祖先，0 根节点，正数表示回复）、`uri`、`cid`、`rkey`、`imageDetails`（CDN URL 数组）、`externalLink`、`videoThumbnailUrl`、`videoPlaylistUrl`、`videoAlt`、`videoAspectRatio`、`quotedPost`（嵌套引用帖结构体）、`isRoot`、`isTruncation`、`likeCount`、`repostCount`、`replyCount`、`indexedAt`。

> 旧版的 `mediaTags` 字段已移除，视频信息通过独立的 `hasVideo` / `videoThumbnailUrl` / `videoPlaylistUrl` / `videoAlt` / `videoAspectRatio` 字段表达。

**设计细节**：`flattenThreadTree()` 将 API 返回的嵌套 `ThreadViewPost` 树压平成基于 depth 的线性数组。`loadThread` 调用 `client.getPostThread(uri, 5, 80)`，并通过 `seedPostViewers(posts)` 批量注入 like/repost 状态。`expandReplies` 增加 `maxSiblings` 后重新展开。回复按时间升序排列。

**新增 `getPostView()`**：通过 `postViewsRef` Map 返回指定 URI 的完整 `PostView`，供信息弹窗等场景使用。

[来源](packages/app/src/hooks/useThread.ts#L46-L137)
...
**说明**：支持**多帖线程式发布**——`posts` 数组中的每个元素对应一个独立帖子，`submit` 按顺序循环提交，后续帖子自动以前帖为 `reply.parent`，形成回复链。
...
**新增 Cache 集成**：通过 `readCache<ProfileCache>(ck)` 在挂载时恢复上次加载的 profile、posts、feedCursor、isFollowing 等信息，条件为 `hasCache(ck)` 时静默加载。Cache key 为 `profile-${actor}`。
...
**新增 Cache 集成**：`initialQuery` 参数支持挂载时自动恢复上次搜索结果。通过 `searchCacheKey(q, t)` 从 `stores/cache.ts` 读取缓存，silent 模式加载不触发 loading 状态。
...
**核心变更**：不再接受 `storage` 参数（自动通过 `chatService.ts` 的 `saveChat`/`loadChat` 管理）。新增 `contextProfile`/`contextPost` 选项替代传统 `contextUri` 参数。`AIConfig` 新增 `visionEnabled`、`thinkingEnabled`、`provider`、`reasoningStyle`、`customSystemPrompt` 字段。
...
[来源](packages/app/src/hooks/useAIChat.ts#L123-L179) | [来源](packages/app/src/services/chatService.ts#L31-L64)
...
[来源](packages/app/src/hooks/useAIChat.ts#L275-L354) | [来源](packages/core/src/ai/assistant.ts#L421-L644)

**自动保存**：两种模式均在消息更新后通过 `autoSave()` 写入 ChatService。`saveChat` 对同一 chatId 做 **300ms 防抖**，写入队列串行化。首次助理回复后，Hook 动态 `import('@bsky/core').generateChatTitle` **异步生成标题**（不阻塞主流程），生成成功后覆盖保存。

[来源](packages/app/src/hooks/useAIChat.ts#L227-L260] | [来源](packages/app/src/services/chatService.ts#L31-L64)

**编辑/回退机制**：`editByIndex(n)` 回滚 `AIAssistant` 内部消息到第 n 条用户消息之前，并返回该消息文本。`undoLastMessage()` 回滚到最后一条用户消息。`edit()` 是 `editByIndex` 的快捷方式（定位最后一条用户消息）。三者均通过 `assistant.loadMessages(keep)` 重新加载裁剪后的消息列表。

[来源](packages/app/src/hooks/useAIChat.ts#L464-L502)

**自动分析**：当 `options.contextProfile` 首次设置且 `messages` 为空时，500ms 延迟后自动发送 `PF_AUTO_ANALYSIS(handle)` 提示词。

[来源](packages/app/src/hooks/useAIChat.ts#L399-L408) | [来源](packages/core/src/ai/prompts.ts#L192-L194)
...
**说明**：`isBookmarked` 是同步 `Set.has()` 操作。`toggleBookmark` 内部判断当前状态后调用 add/remove。书签存储于服务端 `app.bsky.graph.bookmark` 集合，通过 `client.getBookmarks(50)` 加载。**新增 Cache 集成**：通过 `readCache<BookmarkCache>(CACHE_KEY)` 在挂载时恢复 `bookmarks`、`bookmarkedUris` 和 `cursor`。
...
**说明**：获取指定 actor 的所有订阅列表。`actor` 可选，不传则取当前登录用户（通过 `client.getHandle()`）。`createList` 在 API 返回后构造本地 `ListView` 对象并乐观插入列表头部。**新增 Cache 集成**：Cache key 为 `lists-${actor ?? 'self'}`。
...
**说明**：同时管理成员列表和列表 Feed 两种数据，两者独立分页。`load()` 通过 `Promise.all` 并行获取列表详情和 Feed。`toggleMute` 调用 `muteActorList`/`unmuteActorList`。**新增 Cache 集成**：Cache key 为 `listDetail-${listUri}`。
...
`unreadCount` 在客户端计算（`notifications.filter(n => !n.isRead).length`）。通过 `client.listNotifications(30)` 加载，内建一次 1.5 秒重试。**新增 Cache 集成**：Cache key 为 `notifications`，条件为 `hasCache(CACHE_KEY)` 时静默加载。
...
**说明**：不传 `storage` 时自动使用 `getChatStorage()`（来自 `chatService.ts`，TUI 使用 `FileChatStorage`，PWA 使用 `IndexedDBChatStorage`）。`saveConversation` 和 `deleteConversation` 调用后自动 `refresh()` 更新列表。底层通过 `chatService.ts` 统一管理，包含 300ms 防抖写入队列。

[来源](packages/app/src/hooks/useChatHistory.ts#L5-L39] | [来源](packages/app/src/services/chatService.ts#L31-L84)
...
多个 Hook（`useProfile`、`useBookmarks`、`useNotifications`、`useLists`、`useListDetail`、`useSearch`）现在共享同一内存缓存机制：
