...
AI 对话历史和帖子草稿是两种需要持久化的数据。两者共享同一个设计模式：**接口定义契约 → 双平台各自实现 → 消费端零感知**。区别在于：对话存储目前采用硬编码的分发方式，而草稿存储则使用 **工厂模式**，实现了更彻底的平台解耦。
...
graph TB
    subgraph "packages/app (共享层)"
        ...
        useDrafts[useDrafts / createDraftsStore]
    end
    ...
    useChatHistory -->|硬编码| FileChatStorage
    useDrafts -->|工厂分发| FileDraftStorage
    useDrafts -->|工厂分发| IndexedDBDraftStorage
```
...
关联的数据类型包括 `ChatRecord`（完整对话，含 `id`、`title`、`messages: AIChatMessage[]`、时间戳等）和 `ChatSummary`（列表摘要，含 `messageCount`）。`AIChatMessage` 支持 `user`、`assistant`、`tool_call`、`tool_result`、`thinking` 五种角色，并预留了 `reasoning_content` 和 `tool_calls` 字段用于思维链展示。[来源](packages/app/src/services/chatStorage.ts#L5-L38)
...
|------|------------------------|---------------------------|
| 实例化位置 | `useChatHistory.ts` 模块作用域硬编码单例 | `AIChatPage.tsx` / `AIChatWidget.tsx` 中 `useMemo` |
| 存储位置 | `~/.bsky-tui/chats/` 目录 | 浏览器 IndexedDB `bsky-chats` 数据库 |
| 序列化格式 | 独立的 JSON 文件 | IndexedDB 原生对象存储 |
| 平台选择 | 无选择，TUI 只运行在 Node.js | `new IndexedDBChatStorage()` 直接构造 |

注意：与 DraftStorage 不同，ChatStorage **没有使用工厂模式**。`useChatHistory` 内部硬编码了 `new FileChatStorage()`，PWA 端的 `AIChatPage` 和 `AIChatWidget` 则直接 `new IndexedDBChatStorage()`，平台选择由调用方自己负责。[来源](packages/app/src/hooks/useChatHistory.ts#L7-L12), [来源](packages/pwa/src/components/AIChatPage.tsx#L23), [来源](packages/pwa/src/components/widgets/AIChatWidget.tsx#L36)
...
`AppDraft` 包含 `id`、`posts: { text: string }[]`（支持多帖线程）、`replyTo`、`quoteUri`、时间戳，以及关键的 `syncStatus: 'local' | 'synced' | 'modified'` 字段，用于追踪与 PDS 的同步状态。[来源](packages/app/src/services/draftStorage.ts#L5-L14)
...
关键设计在于 `setDraftStorageFactory` + `getDefaultDraftStorage` 这对函数：
...
1. 各平台在**启动入口处**调用 `setDraftStorageFactory` 注册自己的工厂函数。
2. `createDraftsStore` / `useDrafts` 内部通过 `getDefaultDraftStorage()` 获取实例。
3. 工厂函数只执行一次，结果缓存在模块级 `_defaultDraftStorage` 中。
4. 如果没有注册工厂，自动检测 `globalThis.process.versions.node` 判断是否为 Node.js 环境。

这种设计使 `@bsky/app` 核心库**不依赖任何平台特定的存储 API**，同时避免了调用方每次手动传递 storage 实例。[来源](packages/app/src/services/draftStorage.ts#L72-L100)

### 平台注册点

TUI 在 `cli.ts` 入口处注册文件存储：

```typescript
import { setDraftStorageFactory, FileDraftStorage } from '@bsky/app';
setDraftStorageFactory(() => new FileDraftStorage());
```
[来源](packages/tui/src/cli.ts#L15-L17)

PWA 在 `App.tsx` 组件体内注册 IndexedDB 存储：

```typescript
import { IndexedDBDraftStorage } from './services/indexeddb-draft-storage.js';
setDraftStorageFactory(() => new IndexedDBDraftStorage());
```
[来源](packages/pwa/src/App.tsx#L26-L41)

### IndexedDBDraftStorage：PWA 的 IndexedDB 实现

PWA 端的完整实现。数据库名 `bsky_drafts`，对象存储名 `drafts`，主键 `id`：
...
关键设计细节：**无论 PDS 同步是否成功，本地存储总是写入**。这保证了离线状态下草稿不会丢失。`syncStatus` 字段为后续手动同步提供依据。[来源](packages/app/src/hooks/useDrafts.ts#L54-L89)
...
2. 再从 PDS 拉取服务器草稿列表。
3. 对于 PDS 上的每个草稿：如果本地已有匹配的 `serverId`，用服务器数据覆盖本地（**服务器权威**）；如果本地没有，新建本地草稿记录。
