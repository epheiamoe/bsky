...
- **滚动位置丢失修复** (v0.10.4, Issue #7): 模块级 `_heightCache`（`post.uri → 实际高度`）跨 mount 持久；`estimateSize` 优先读缓存消除 120px vs ~170px 的累积偏移；`useLayoutEffect` 替换 `requestAnimationFrame`，scrollTop 恢复在 paint 前同步完成。
