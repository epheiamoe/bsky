...
- 🔄 useModerationPipeline 死代码: 保留但不使用（所有列表组件使用 `useModerationBatch`）
  - 待后续决策：连接 Pipeline 实现 block 策略，或删除死代码
  - 支持按 did 单独 dismiss，失败恢复后自动重置 dismiss 状态
  - 有 refresh 能力的组件支持 onRetry 重新 moderation batch
  - Commit: `861df44`

### 待完成:
- Playwright 验证 staging（compose page paste、FeedTimeline scroll、moderation banner dismiss）
- 部署到生产环境
