...
- ✅ Per-feed 内存缓存 (FeedCache): 每个 feed 独立缓存帖子 + cursor + 滚动位置，滑动窗口 LRU 删除（默认上限 1000 条，设置可配 100-5000）
- ✅ 缓存后切换 feed 无 loading 闪烁: 帖子瞬间显示，直接恢复滚动位置
- ✅ 缓存满提示: 底部显示「已缓存 N 条帖子 — 刷新以查看更新内容」+ 刷新按钮
