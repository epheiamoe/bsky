...
- 🔄 Per-feed 内存缓存 (FeedCache): 已实现但仍有 bug
  - 每个 feed 独立缓存帖子 + cursor + 滚动位置，滑动窗口 LRU 删除（默认上限 1000 条）
  - Playwright 测试显示：切换 feed 时内容仍然不正确，需要进一步调试
  - 可能原因：缓存恢复时机、虚拟滚动初始化、或 FeedTimeline 简化过度

### 待完成:
- 修复 per-feed cache 切换时的内容显示问题
- Playwright 自动化测试确认修复后部署
