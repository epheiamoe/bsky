...
> **Depends on**: v0.15.0 hotfixes (current)  
> **Created**: 2026-05-20  
...
The current Phase 14 implementation (v0.15.0) has a **fundamental architectural flaw**: the PWA Worker (`pyodide.worker.ts`) re-implemented all 27 read API handlers from scratch instead of reusing the existing `tools.ts` handlers.
