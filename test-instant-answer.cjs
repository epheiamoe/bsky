...
// Usage: node test-instant-answer.cjs

const core = require('./packages/core/dist/index.js');
const { createTools, AIAssistant, P_ASSISTANT_BASE, PF_CURRENT_TIME, P_CONCISE } = core;
...
for (const rawQuery of queries) {
  ...
  try {
    ...
    if (data.Abstract) parts.push(`Abstract: ${data.Abstract.slice(0, 80)}...`);
    ...
    if (data.RelatedTopics?.length > 0) {
      const flatTopics = data.RelatedTopics.slice(0, 8).flatMap(t => t.Topics || [t]);
...
try {
  ...
  console.error(`\n❌ AIAssistant error: ${err.message}`);
  if (err.cause) console.error(`   Cause:`, err.cause);
}

// ── Step 3: Test multiple queries through LLM ──
console.log('\n══════════════════════════════════════════');
